...
function handleAPI(req, res) {
  ...
  if (req.url === '/api/stats' && req.method === 'GET') {
    ...
    console.log(`  → API: GET /api/stats → 200 (${mockStats.sessions.length} sessions)`);
    return true;
  }
  ...
  if (req.url === '/api/stats/visit' && req.method === 'POST') {
    ...
    console.log(`  → API: POST /api/stats/visit → 200 (total: ${mockStats.totalVisitors})`);
    return true;
  }
  ...
  if (req.url === '/api/stats/web-heartbeat' && req.method === 'POST') {
    ...
    console.log(`  → API: POST /api/stats/web-heartbeat → 200`);
    return true;
  }
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('  ┌──────────────────────────────────────────┐');
  console.log('  │                                          │');
  console.log('  │   MSH2LM Local Server                    │');
  console.log('  │                                          │');
  console.log(`  │   Local:    http://localhost:${PORT}          │`);
  console.log(`  │   Network:  http://192.168.1.18:${PORT}      │`);
  console.log('  │                                          │');
  console.log('  │   ✅ Static files + Mock API ready       │');
  console.log('  │                                          │');
  console.log('  └──────────────────────────────────────────┘');
  console.log('');
});
